...
class BatchInfer:
    ...
    def __init__(self, model: EmbedModel, batch_size: int, worker_count: int, sub_worker_count: int):
        """
    ...
    def _worker(self, rank: int, datasets: Sequence[IntervalDataset], out_paths: Sequence[str], post: Sequence[Callable[[np.memmap], None]] | None):
        ...
        master_out_file = np.memmap(master_out_path, np.float32, 'r+', offset, shape=(sample_count, e_dim))
        data_loader = DataLoader(master_dataset, batch_size=self.batch_size, sampler=block_sampler, shuffle=False, pin_memory=True, num_workers=self.sub_worker_count, persistent_workers=self.sub_worker_count != 0, collate_fn=collate)
        try:
            os.environ['MASTER_ADDR'] = 'localhost'
            os.environ['MASTER_PORT'] = '12356'
            dist.init_process_group(backend='nccl' if torch.cuda.is_available() else 'gloo', rank=rank, world_size=self.worker_count, timeout=timedelta(seconds=len(master_dataset) / self.worker_count * 0.5), device_id=device if device.type == 'cuda' else None)
    ...
    def batch(self, datasets: Sequence[ListLike], out_paths: Sequence[str], post: Sequence[Callable[[np.memmap], None]] | None=None):
        """
